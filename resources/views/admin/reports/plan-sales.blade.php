...
<div class="card mb-4 border-0 shadow-sm">
  <div class="card-header border-0 d-flex align-items-center justify-content-between flex-wrap gap-3">
    ...
    <a href="{{ route('admin.reports.plan-sales', array_merge(request()->query(), ['export' => 'excel'])) }}" class="btn btn-success">
  ...
  <div class="card-body">
    ...
    <div class="alert alert-info d-flex align-items-center gap-2 mb-4">
      ...
      <div>
        <strong>إجمالي المبيعات:</strong> <span class="fw-bold text-info">{{ number_format(($total ?? 0)/100, 2) }} {{ $payments->first()?->currency ?? 'SAR' }}</span>
      </div>
    </div>
        ...
        <tbody>
          @forelse($payments as $p)
            <tr>
              ...
              <td>
                <span class="fw-semibold">{{ $p->userRequest?->trainer?->name ?? '—' }}</span>
              </td>
              <td>
